...
This study analyzes probabilistic seasonal and sub-seasonal forecasts of heavy rainfall events, defined by exceedance of the \nth{90} percentile of modeled rainfall.
...
Calculation of the classifiability index for several values of $K$ (supplemental figure S3)\footnote{CHECK} suggests that states with $K=5,6,\ldots,8$ are all reasonable.
...
At $t=-2$ a mid-latitude baroclinic system approaches the South American continent, intensifying and moving to the East at $t=-1,0,1$ days.
...
At timescales of one to three weeks, the model successfully forecast the strongest breaks and pauses in the rainfall, such as the heavy rainfall during December 2015 and the dry period during mid-January 2016.
...
These skill scores indicate that the model is capturing a signal, and suggest the use of \glspl{mos} to explore the extent to which corrections in the magnitudes and spatial patterns may improve the forecast.
...
Numerical forecasts skillfully predicted enhanced risk of heavy rainfall at the seasonal scale, likely guided by the \gls{enso} signal, but biases in the spatial patterns suggest that models are not well capturing the physical interactions between the Pacific and the Atlantic basins.
