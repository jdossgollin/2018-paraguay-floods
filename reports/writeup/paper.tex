...
\Glspl{wt} 5 and 3 look loosely inverses of \glspl{wt} 1 and 4, respectively, and are associated with dry anomalies over the Lower Paraguay River Basin.
...
Compared to the uncorrected \gls{s2s} model forecast and the logistic regression models, \gls{pcr} and \gls{cca} models perform quite well (\cref{fig:subs-prob-fcst}, middle and bottom row).\footnote{Letters}
...
Although many of the individual rainfall events of NDJF 2015-16 were intense, they were nontheless driven by the climatological mechanism for heavy rainfall and intense convection shown in \cref{fig:lagged-rain} and discussed in more detail in previous studies \citep{Saulo2007,Salio2007,Marengo2004,Velasco1987}.
...
Improved understanding of the physical mechanisms, and their cross-timescale interactions, that led to this behavior may support both improved disaster preparedness \citep{Hellmuth2011,Goddard2014} and improved model forecast skill, particularly at sub-seasonal timescales which is an area of active research interest.
...
A persistent dipolar \gls{sst} anomaly in the central southern Atlantic Ocean favors the occurrence of \gls{wt} 4 by blocking transient extratropical wave activity coming from the Pacific, facilitating transitions from Chaco jet events (\gls{wt} 1) to No-Chaco jet events (\gls{wt} 4) via enhanced low-level wind circulation from southern Brazil towards the Atlantic, and back to north-east Brazil and the Amazon (see \cref{fig:chaco-nochaco}) due to land-sea temperature contrasts.
...
\begin{figure*}
	...
	\caption{
		Chiclet diagram \citep{Carbin:2016fx} of ensemble-mean precipitation anomaly forecasts over the Lower Paraguay River Basin (see \cref{fig:study-area}) from uncorrected \gls{ecmwf} \gls{s2s} model forecast data, as a function of the forecast target date (horizontal axis) and lead time (vertical axis).
...
\begin{figure*}
	...
	\caption{
		...
		When a dipolar SST anomaly occurs in the central southern Atlantic with the warmer pole equatorward, the meridional temperature gradient and sea-land temperature contrasts establish an anticyclonic circulation (dot-dashed line) conducive to increased occurrence of No-Chaco jet events (b).
