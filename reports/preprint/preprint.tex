...
\author[1,2]{James Doss-Gollin\thanks{\href{mailto:james.doss-gollin@columbia.edu}{james.doss-gollin@columbia.edu}}}
\author[3,4]{\'{A}ngel G. Mu\~{n}oz}
\author[4]{Simon J. Mason}
\author[5,6]{Max Past\'{e}n}
\affil[1]{Department of Earth and Environmental Engineering, Columbia University}
\affil[2]{Columbia Water Center}
\affil[3]{Atmospheric and Oceanic Sciences (AOS), Princeton University}
\affil[4]{International Research Institute for Climate and Society (IRI), The Earth Institute, Columbia University.}
\affil[5]{Direcci\'{o}n de Meteorolog\'{i}a e Hidrolog\'{i}a, Paraguay}
\affil[6]{Facultad Polit\'ecnica, Universidad Nacional de Asunci\'{o}n, Paraguay}

\begin{document}

This is a post-print of the paper ``Heavy rainfall in Paraguay during the 2015-16 austral summer: causes and sub-seasonal-to-seasonal predictive skill'', by James Doss-Gollin, \'{A}ngel G. Mu\~{n}oz, Simon J. Mason, and Max Past\'{e}n, published in Journal of Climate.
You can find the final, type-set version at \url{https://journals.ametsoc.org/doi/abs/10.1175/JCLI-D-17-0805.1} or see the full citation reference in the bibliography under \citet{DossGollin:2018bn}.

Codes used to build this paper are available at \url{https://github.com/jdossgollin/2018-paraguay-floods}.
Comments to the authors may be directed to James Doss-Gollin: \href{mailto:james.doss-gollin@columbia.edu}{james.doss-gollin@columbia.edu}
...
\begin{abstract}
	...
	The simultaneous use of seasonal and sub-seasonal heavy rainfall predictions could have provided decision makers useful information about the start of these flooding events from two to four weeks in advance.
	...
	A continuous monitoring of climate drivers impacting rainfall in the region, and the use of statistically corrected heavy precipitation seasonal and sub-seasonal forecasts, may help improve flood preparedness in this and other regions.
\end{abstract}
...
\begin{figure}
	\centering
	\includegraphics[width=5in]{study_area.jpg}
	\caption{
...
\begin{figure}
	\centering
	\includegraphics[width=6.5in]{anomalies_ndjf1516.pdf}
	\caption{
...
Thus, upstream of the Pantanal the streamflow maxima typically occurs in phase with precipitation, while downstream of the Pantanal - an area which we define in \cref{fig:study-area} as the \acrlong{lprb} - the annual peak typically occurs between April and July.
...
This study analyzes probabilistic seasonal and sub-seasonal forecasts of heavy rainfall events, which we define as exceedance of the \nth{90} percentile of NDJF daily precipitation across all ensemble members and initializations.
...
Computation was performed in the python environment using stable open source packages \citep{Hunter2007,McKinney2010,vanderWalt2011,Hoyer2017}.
All codes to reproduce or modify this analysis are available at the permanent link \url{https://doi.org/10.5281/zenodo.1243104}.
...
Though our approach of first selecting the number of \glspl{eof} to use and then choosing to scale them equally involves more subjective decisions than an approach without rescaling, in this case the resulting physical patterns described by the \glspl{eof} more closely represent patterns identified in the literature; this is further discussed in \cref{sec:diagnostics}.
...
Logistic regression models the probability of binary events, conditional on one or more predictors, and has been widely used in \gls{mos}.
...
The \gls{xlr} was designed to address this shortcoming via the consideration of a transformation of the thresholds of interest as an additional predictor variable \citep{Wilks2009}.
...
As discussed in \citet{Barros2004,Bravo2011}, the location of the Bah\'ia Negra gauge (see \cref{fig:study-area}) in the Pantanal region means that it responds very slowly to rainfall input.
...
As the system progresses, the jet below \SI{20}{\degree S} transitions from predominantly meridional flow (``Chaco Jet''; $t=\SI{-1}{\day}$) to predominantly zonal flow (``No-Chaco Jet''; $t=\SI{0}{\day}$).
...
\begin{figure}
	\centering
	\includegraphics[width=6.5in]{lagged_rain.pdf}
	\caption{
...
\begin{figure}
	\centering
	\includegraphics[width=6.5in]{eof_loadings.pdf}
	\caption{
...
\begin{figure}
	\centering
	\includegraphics[width=6.5in,height=9in,keepaspectratio=true]{wt_composite.pdf}
	\caption{
...
\Gls{wt} 4 also shows \gls{sallj} activity, but the wind turns to the East northward of \SI{25}{\degree S}, leading to heavy rainfall over Eastern Paraguay and SW Brazil; this has been called a ``No-Chaco'' Jet event \citep{Vera2006}.
...
\begin{figure}
	\centering
	\includegraphics[width=6.5in]{rain_wt_201516.pdf}
	\caption{
...
\begin{figure}
	\centering
	\includegraphics[width=3.5in]{seasonal_forecast.pdf}
	\caption{
...
Nonetheless, the international \gls{s2s} Prediction Project \citep{Vitart2016} provides free access to almost-real-time sub-seasonal forecasts from multiple models, an opportunity to explore how well the heavy rainfall events of the first week of December 2015 could have been predicted.
...
\begin{figure}
	\centering
	\includegraphics[width=6.5in]{chiclet.pdf}
	\caption{
...
\begin{figure}
	\centering
	\includegraphics[width=6.4in]{mos_forecasts.pdf}
	\caption{
		Raw and \gls{mos}-adjusted \gls{s2s} model forecasts and skill scores for the methods indicated in \cref{tab:mos-methods}.
...
\begin{figure}
	\centering
	\includegraphics[width=6.4in]{ChacoNoChacojet.pdf}
	\caption{
...
From a policy perspective, reducing flood risk exposure in this region is key to reducing flood losses.
Flood events not only in 2015-16 but also in 2014, 2017, and 2018 have caused substantial damage, and highlight the need for flood risk management strategies.
